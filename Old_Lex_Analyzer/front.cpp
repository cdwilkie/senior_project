...
/* front.cpp - Lexical analyzer system for simple
 * arithmetic expressions ( mul. & div)
 * Implements a getChar(), addChar(), and nextChar()
 * function to build char array lexeme[].  Lexical
 * analyzer lex() generates token codes for each 
 * lexeme[].
 */
...
/*lookup - Function to lookup lexeme symbols
* and determine their token type.  Type is 
* stored in nextToken.
*/
int lookup (char ch) {
    switch (ch) {
